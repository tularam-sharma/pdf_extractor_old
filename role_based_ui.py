...
class TemplateManagementCard(RoleBasedWidget):
    ...
    def setup_ui(self):
        ...
        description_label = QLabel(
            ...
            "Configure headers, table regions, and summary sections."
        )
...
class RulesManagementCard(RoleBasedWidget):
    """Card widget for rules management (developers only)."""
    clicked = Signal()
    
    def __init__(self, parent=None):
        super().__init__(required_permission="rules_management", parent=parent)
        self.setup_ui()
    
    def setup_ui(self):
        """Set up the rules management card UI."""
        self.setStyleSheet("""
            QWidget {
                background-color: white;
                border-radius: 8px;
        ...
        # Subtitle
        subtitle_label = QLabel("Define and manage extraction rules")
        subtitle_label.setFont(QFont("Arial", 12))
        subtitle_label.setStyleSheet("color: #666;")
        subtitle_label.setAlignment(Qt.AlignCenter)
        subtitle_label.setWordWrap(True)
        ...
        # Description
        description_label = QLabel(
            "Create and maintain rules for data extraction and validation. "
            "Set up conditions for automatic field mapping and data transformation."
        )
        description_label.setWordWrap(True)
        description_label.setStyleSheet("color: #666;")
        description_label.setAlignment(Qt.AlignCenter)
        ...
        # Action button
        self.action_button = QPushButton("Manage Rules")
        self.action_button.clicked.connect(self.clicked.emit)
        button_layout.addWidget(self.action_button, alignment=Qt.AlignCenter)
...
class MainDashboard(QWidget):
    ...
    def setup_ui(self):
        ...
        self.cards_layout.setSpacing(20)  # Add space between cards
        ...
        # Rules Management card (developers only)
        self.rules_card = RulesManagementCard()
        self.rules_card.set_user_management(self.user_management)
        self.rules_card.clicked.connect(self.show_rules_manager.emit)
        ...
        self.cards = [self.upload_card, self.template_card, self.rules_card, self.bulk_card]
    ...
    def update_dashboard(self):
        ...
        if is_logged_in:
            ...
            self.rules_card.update_visibility()
...
class RoleBasedPDFProcessor(RoleBasedWidget):
    ...
    def load_pdf_file(self, filepath):
        ...
        if self.pdf_processor.isVisible():
            self.pdf_processor.load_pdf_file(filepath)
